...
```{r}
url <- "https://raw.githubusercontent.com/k-state-id3a/mixed-models-fall26/refs/heads/newpart3/data/example1_part3.csv"
d1 <- read.csv(url)
...
```{r}
m1 <- glmmTMB(severity ~ fungicide + (1|block), 
...
```{r}
m1_2 <- glmmTMB(severity ~ fungicide + (1|block), 
...
```{r}
m2 <- glmmTMB(cbind(germ, n-germ) ~ extract*gen, 
